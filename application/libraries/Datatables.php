...
//                        $sWhere .= $this->select[$mColArray[$i]['data']] . " ILIKE '%" . $sSearch . "%' OR ";
//                    } else {
////                        $sWhere .= $this->select[$this->columns[$i]] . " I"
////                                . "LIKE '%" . $sSearch . "%' OR ";
//                        $sWhere .= $this->select[$this->columns[$i]] . " I"
//                                . "LIKE '%" . $sSearch . "%' OR ";
//                    }
...
//    }
    private function get_filtering() {
        ...
        $search = $this->ci->input->post('search'); //filtro principal
        ...
        if ($sSearch != '') {
            for ($i = 0; $i < count($mColArray); $i++) {
                if ($mColArray[$i]['searchable'] == 'true') {
                    if ($this->check_cType()) {
                        $sWhere .= "CAST(" . $this->select[$mColArray[$i]['data']] . " AS VARCHAR) ILIKE '%" . $sSearch . "%' OR ";
                    } else {
                        $sWhere .= "CAST(" . $this->select[$this->columns[$i]] . " AS VARCHAR) I"
                                . "LIKE '%" . $sSearch . "%' OR ";
                    }
                }
            }
        ...
        if ($sSearch == '') {
            if (count($mColArray)) {
                foreach ($mColArray as $key => $value) {
                    if ($value['search']['value'] != '') {
                        if ($this->check_cType()) {
                            $sWhere .= "CAST(" . $this->select[$mColArray[$key]['data']] . " AS VARCHAR) ILIKE '%" . $value['search']['value'] . "%' AND ";
                        } else {
                            $sWhere .= "CAST(" . $this->select[$this->columns[$key]] . " AS VARCHAR) ILIKE '%" . $value['search']['value'] . "%' AND ";
                        }
                    }
                }
    ...
    private function produce_output($output, $charset) {
        ...
        $aaData = $this->rectificarJson($aaData);
    ...
    private function rectificarJson($data) {
        $datos = array();
        if ($data && is_array($data)) {
            foreach ($data as $i => $value) {
                foreach ($value as $key => $val) {
                    $arreglo[$key] = ($val == NULL) ? '' : $val;
                }
                $datos[] = $arreglo;
                $arreglo = array();
            }
        }
        $respuesta = (COUNT($datos) > 0) ? $datos : '';
        return $respuesta;
    }
    ...
    private function get_total_results($filtering = FALSE) {
        ...
        $this->ci->db->select($cols, FALSE);
//        $query = $this->ci->db->count_all($this->table);
...
//      echo $column[0]['data'];
    ...
    public function debug($tabla) {
        return $this->ci->db->get_compiled_select($tabla);
    }
